...
function update_script() {
  ...
  UPD=$(whiptail --backtitle "WDGIS Proxmox VE Helper Scripts" --title "UPDATE" --radiolist --cancel-button Exit-Script "Spacebar = Select" 11 58 4 \
    "1" "Update Core" ON \
    "2" "Install HACS" OFF \
  ...
  header_info
  if [ "$UPD" == "1" ]; then
    if (whiptail --backtitle "WDGIS Proxmox VE Helper Scripts" --defaultno --title "SELECT BRANCH" --yesno "Use Beta Branch?" 10 58); then
      clear
      header_info
