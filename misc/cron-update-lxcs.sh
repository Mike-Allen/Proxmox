...
# bash -c "$(wget -qLO - https://github.com/Mike-Allen/Proxmox/raw/Localize/misc/cron-update-lxcs.sh)"
...
add() {
  ...
  sh -c '(crontab -l -u root 2>/dev/null; echo "0 0 * * 0 PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin /bin/bash -c \"\$(wget -qLO - https://github.com/Mike-Allen/Proxmox/raw/Localize/misc/update-lxcs-cron.sh)\" >>/var/log/update-lxcs-cron.log 2>/dev/null") | crontab -u root -'
  clear
  echo -e "\n To view Cron Update LXCs logs: cat /var/log/update-lxcs-cron.log"
}

remove() {
  (crontab -l | grep -v "github.com/Mike-Allen/Proxmox/raw/Localize/misc/update-lxcs-cron.sh") | crontab -
...
CHOICE=$(whiptail --backtitle "WDGIS Proxmox VE Helper Scripts" --title "Cron Update LXCs" --menu "Select an option:" 10 58 2 \
  "${OPTIONS[@]}" 3>&1 1>&2 2>&3)
