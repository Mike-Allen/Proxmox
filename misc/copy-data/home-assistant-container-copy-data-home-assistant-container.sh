...
while [ -z "${CTID_FROM:+x}" ]; do
  CTID_FROM=$(whiptail --backtitle "WDGIS Proxmox VE Helper Scripts" --title "$TITLE" --radiolist \
    "\nWhich HA LXC would you like to copy FROM?\n" \
    16 $(($MSG_MAX_LENGTH + 23)) 6 \
    "${CTID_MENU[@]}" 3>&1 1>&2 2>&3) || exit
done
while [ -z "${CTID_TO:+x}" ]; do
  CTID_TO=$(whiptail --backtitle "WDGIS Proxmox VE Helper Scripts" --title "$TITLE" --radiolist \
...
whiptail --backtitle "WDGIS Proxmox VE Helper Scripts" --defaultno --title "$TITLE" --yesno \
...
# Use to copy all data from one Home Assistant LXC to another
# run from the Proxmox Shell
# bash -c "$(wget -qLO - https://raw.githubusercontent.com/Mike-Allen/Proxmox/Localize/misc/copy-data/home-assistant-container-copy-data-home-assistant-container.sh)"
